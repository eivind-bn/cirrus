...
import cirrus.event.{Delegator, Reporter}
...
class ReporterTest extends AnyWordSpec {
  ...
  "A reporter" should {
    ...
    "methods behaviour meets expectations" in {
      ...
      val delegator = Delegator[Int]
      var block = false
      root.flatMap(_ => delegator).filterNot(_ => block).tapEach(i => counter += i).dispatch(0)
      for(i <- 1 to 250) delegator.dispatch(i)
      def increasingNaturalsSum(n: Int): Int = (n*(n+1))/2
      expected += increasingNaturalsSum(250)

      root.map(_ => delegator).flatten.drop(500).tapEach(i => counter += i).dispatch(0)
      block = true
      for(i <- 1 to 750) delegator.dispatch(i)
